...
By convention Peon will look for `.mesh_material` file found side-by-side textures to determine the role of each texture. This will be generated by [MeshBuilder](#MeshBuilder). For example [advent_interceptor_strikecraft.mesh_material](./examples/meshes/textures/advent_interceptor_strikecraft.mesh_material)

1. Download [Texconv](https://github.com/Microsoft/DirectXTex/wiki/Texconv)

2. Run Peon with path to Texconv.exe
```
py run-peon.py -s .\examples\meshes\textures\ -d .out\convert_textures_example\ --texconv_exe_path c:/path/to/your/texconv.exe
```
